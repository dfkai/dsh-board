...
- 本地每日预算制动（localStorage 设定 + 徽章/面板进度条，80% 黄 / 超额红；参考 dsh-balance-stats 的交互，本地实现、零外发）
- 本月用量统计（现为 本日/本周）
- 面板健康提示已落地（缓存命中率 < 50% 提示）；如需按会话级体检再议

## 明确不吸收（社区合并讨论立场）

- 官方余额 / 续航 / 账单对账明细（dsh-balance-stats 的核心）：需要用户 API key + 向 api.deepseek.com 外发请求，与 dsh-board「零外发、零密钥」承诺冲突——不并入核心；如社区合并，可作为**可选卫星插件**保留
- 顶栏内联价目浮层（dsh-client-pricing 的主 UI）：与侧栏面板定位重复；已吸收其核心价值（峰谷/现行三态 + 当前输出价）为徽章角标
