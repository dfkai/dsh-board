...
with sync_playwright() as p:
    ...
    print(page.locator('.dsh-board-panel').first.inner_text())
    print(f"spark bars: {page.locator('.dsh-board-spark rect').count()}")
    print('--- console errors ---')
    for line in errors[:10]:
        print(line)
    page.screenshot(path='/tmp/dsh-board-3081-after-turn.png', full_page=False)
    browser.close()
