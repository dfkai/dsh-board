...
export const zh = {
  ...
  'trend.tooltip': '第 {turn} 轮 · 入 {in} · 出 {out}',
  'heat.day': '{date} · {tokens} token',
  'spark.empty': '暂无数据',
  'note.pricing': '按官方价目估算（含思考 token；累计按各会话主导模型计价，可能与平台账单有出入）',
  'window.peak': '⛰️ 高峰价',
  'window.offpeak': '🌙 闲时价',
  'chip.standard': '现行一口价',
  'chip.peak': '⛰️ 高峰',
  'chip.offpeak': '🌙 闲时',
  'chip.rate': '出 ¥{price}/M',
  'hint.cacheLow': '缓存命中率仅 {percent}%——一次未命中约等于 {ratio} 次命中的价钱，反复读取大文件会显著多花。',
  'rank.0': '🌱 未醒词芽',
  'rank.1': '🥉 词途学徒',
  'rank.2': '💬 白银之舌',
  'rank.3': '💰 一词千金',
...
export const en: Record<RichKey, string> = {
  ...
  'trend.tooltip': 'turn {turn} · in {in} · out {out}',
  'heat.day': '{date} · {tokens} tokens',
  'spark.empty': 'No data yet',
  'note.pricing': "Official list prices; reasoning included; lifetime priced per session's dominant model — may differ from platform billing",
  'window.peak': '⛰️ peak',
  'window.offpeak': '🌙 off-peak',
  'chip.standard': 'flat rate',
  'chip.peak': '⛰️ peak',
  'chip.offpeak': '🌙 off-peak',
  'chip.rate': 'out ¥{price}/M',
  'hint.cacheLow': 'Cache hit rate is only {percent}% — one miss costs ~{ratio} hits; re-reading large files adds up.',
